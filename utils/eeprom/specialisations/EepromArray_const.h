...
namespace eeprom
{
	...
	/// @brief
	/// A specialisation of @sref{eeprom::EepromArray<Type\, capacity>, EepromArray<Type\, capacity>}
	/// that represents an array of read-only objects.
	template<typename Type, size_t capacity>
	class EepromArray<const Type, capacity>
	{
	public:
		/// @brief
	...
	public:
		...
		/// because @sref{eeprom::EepromArray<const Type\, capacity>, EepromArray<const Type\, capacity>}
	...
	/// which can lead to <strong>undefined behaviour</strong>.
	template<typename Type, size_t capacity>
	constexpr EepromArray<const Type, capacity> makeEepromArray(const Type (&array)[capacity])
	{
		return array;
	}
}